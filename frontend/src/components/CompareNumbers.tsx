...
interface Props {
  ...
  /** Annual household kWh from lead (briefing); drives grid-independence denominator. */
...
function buildRows(annualHouseholdKwh: number) {
  const h = annualHouseholdKwh > 0 ? annualHouseholdKwh : HOUSEHOLD_DEFAULT_KWH;
  return [
    {
      label: "System size",
      explainKey: "systemSize" as const,
      icon: (
      ...
      getValue: (o: OfferData) => `${o.offer.system_kwp.toFixed(1)} kWp`,
    },
    {
      label: "Annual production",
      explainKey: "annualProduction" as const,
      icon: (
        <svg className="w-4 h-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg>
      ),
      getValue: (o: OfferData) => fmtNum(o.offer.annual_production_kwh, "kWh/yr"),
    },
    {
      label: "Self-consumption",
      explainKey: "selfConsumption" as const,
      icon: (
        <svg className="w-4 h-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M13 10V3L4 14h7v7l9-11h-7z" />
        </svg>
      ),
      getValue: (o: OfferData) => `${Math.round(o.offer.self_consumption_pct)}%`,
    },
    {
      label: "Grid independence",
      explainKey: "gridIndependence" as const,
      icon: (
        <svg className="w-4 h-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
    ...
    },
    {
      ...
      explainKey: "annualSavings" as const,
      icon: (
        <svg className="w-4 h-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
        </svg>
      ),
      getValue: (o: OfferData) => fmt(o.offer.annual_savings_eur),
    },
    {
      label: "Payback",
      explainKey: "payback" as const,
      icon: (
        <svg className="w-4 h-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      ),
      getValue: (o: OfferData) => `${o.offer.payback_years} years`,
    },
    {
      label: "CO₂ saved",
      explainKey: "co2Saved" as const,
      icon: (
        <svg className="w-4 h-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
      ),
      getValue: (o: OfferData) => fmtNum(o.offer.co2_saved_kg, "kg/yr"),
    },
    {
      label: "Roof utilization",
      explainKey: "roofUtilization" as const,
      icon: (
        <svg className="w-4 h-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
    ...
    },
    {
      ...
      explainKey: "twentyYearSavings" as const,
      icon: (
        <svg className="w-4 h-4 text-muted-foreground" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
...
export default function CompareNumbers({ offers, enrichment, annualHouseholdKwh }: Props) {
  const household = annualHouseholdKwh ?? HOUSEHOLD_DEFAULT_KWH;
  const rows = useMemo(() => buildRows(household), [household]);
  const exampleOffer =
                ...
                <td className="py-3 pr-3">
                  <div className="flex items-center gap-1.5">
                    ...
                    <InfoTooltip
                      content={
                        exampleOffer
                          ? buildCompareRowContent(row.explainKey, exampleOffer, enrichment, household)
                          : "No offers loaded."
                      }
